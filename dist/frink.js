...
function _selectImpl(node, path) {
	...
	var nodelist = _seq.isSeq(node) ? node = _transducers.transform(node, _transducers.compose(_transducers.forEach(n => axis.f(n)), _transducers.cat)) : axis.f(node);
	return _transducers.transform(nodelist, _transducers.compose( /*filter(nodeFilter),*/_transducers.forEach(process), n => isNode(n) ? attr ? _transducers.cat(n) : _transducers.distinctCat(_comparer())(n) : n));
}
...
function _n(type, name, children = []) {
	if (_seq.isSeq(children)) children = children.toArray();
