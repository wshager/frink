			...
			if (len > 0) {
				...
				for (let i = 0; i < len; i++) {
					...
					if (_ !== undefined) {
						...
						// TODO nested groups
						if (_ !== "") {
							let ge = _vnode.e("fn:group", _seq.seq(_vnode.a("nr", Number(i + 1)), _vnode.x(_)));
							children.push(ge);
							me = _modify.appendChild(me, ge);
						} else {
							var clen = children.length;
							if (clen) {
								var last = children[clen - 1];
								last = _modify.appendChild(last, _vnode.e("fn:group", _seq.seq(_vnode.a("nr", Number(i + 1)))));
							}
						}
					}
				}
...
function matches($str, $pat) {
	...
	str = _access.isNode(str) ? str.data() : str;
