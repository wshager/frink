...
function analyzeString($str, $pat) {
	...
	var ret = _construct.e("fn:analyze-string-result");
	var index = 0;
	if (str) {
		...
		_xregexp2.default.replace(str.toString(), _cached(pat), function (...args) {
			...
			var idx = args.pop();
			// the rest is groups
			if (idx > index) ret = _modify.appendChild(ret, _construct.e("fn:non-match", _construct.x(str.substring(index, idx))));
			index = idx + match.length;
			var len = args.length;
			var me = _construct.e("fn:match");
			if (len > 0) {
				var children = [];
				for (let i = 0; i < len; i++) {
					let _ = args[i];
					if (_ !== undefined) {
						...
						// TODO nested groups
						if (_ !== "") {
							children[i] = _construct.e("fn:group", _seq.seq(_construct.a("nr", Number(i + 1)), _construct.x(_)));
						} else {
							var clen = children.length;
							if (clen) {
								var last = children[clen - 1];
								last = _modify.appendChild(last, _construct.e("fn:group", _seq.seq(_construct.a("nr", Number(i + 1)))));
							}
						}
					}
				...
				me = _modify.appendChild(me, _construct.x(match));
				ret = _modify.appendChild(ret, me);
			}
		});
		if (index < str.length) ret = _modify.appendChild(ret, _construct.e("fn:non-match", _construct.x(str.substr(index))));
	}
	return ret;
}
...
function replace($str, $pat, $rep) {
	...
	return _type.string(_xregexp2.default.replace(str.valueOf(), _cached(pat), _repcached(rep), "all"));
}
...
function concat(...args) {
	return _type.string(_transducers.transform(args, _transducers.compose(_transducers.forEach(s => _type.string(s)), _transducers.cat)).join(""));
}
