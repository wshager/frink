...
function _formNodeToSchema(node) {
	...
	var type = _inferTypeFromFormat(s.format);
	if (type) {
		s.type == type;
		delete s.format;
	}
	if (inode.type == "select-one") {
		s.enum = [].concat(_toConsumableArray(inode.options)).map(function (o) {
			return o.value;
		});
	}
	return s;
...
function validate($node, $schema) {
	...
	if (params.form) {
		return (0, _access.parent)($node).concatMap(function (node) {
			return _validate(node, _formNodeToSchema(node), params);
		});
	}

	return $node.concatMap(function (node) {
		return (0, _seq.zeroOrOne)($schema).concatMap(function (schema) {
			return _validate(node, schema, params);
		});
	});
}
...
var validator = {
	...
	minLength: function minLength(schema, key, params, index, path, err, node) {
		...
		if ((0, _util.ucs2length)(node.value) < test) err.push(x(schema, key, params, path + "/" + index, node));
	},
