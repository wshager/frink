...
function x(schema, key, params, path, node, faults) {
	var validationMessage = params.form ? node.get("validationMessage") : "";
...
const types = {
	...
	integer: function (node) {
		...
		if (val.constructor != Big.default) val = new Big.default(val);
		return node.type == 3 && val.constructor == Big.default && val.e === 0;
	},
...
const validator = {
	...
	minimum: function (schema, key, params, index, path, err, node) {
		var test = schema[key];
		var ret = false;
		if (node.value && node.value.constructor == Big.default) {
