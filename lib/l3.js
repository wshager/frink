...
class Close {
	...
	serialize() {
		...
		} else if(type == IF || ((type == THEN || type == ELSE) && node.count() == 1)) {
			ret = "";
		} else {
			ret =  ")";
		}
		return ret + (node.parent.type == 15 && node.indexInParent < node.parent.count() ? "," : "");
	}
}
	...
	serialize() {
		if(this.type == VAR) {
			if(this.$node.count() > 1) {
				return "$" + this.name + " = " + (this.ref.length > 1 ? "replay(" : "") + "fromPromise(";
			} else {
				return "$" + this.name + (this.$node.indexInParent < this.$node.parent.count() ? "," : "");
			}
		...
		if (type == 17) {
			...
			if(node.node == cur.$node) {
				if(cur.type == QUOT) {
					...
					//console.log(assigs.map(x => x+"(refs:"+x.ref.map(x => x+"")+")"));
					let txt = noIfAndOr ? "" : "async ";
					txt += "(" + Array.from(Array(size).keys()).map(x => "$" + (x + 1)) + ") => {\n";
					txt += "let " + assigs.join(",") + ";\nreturn ";
					const count = cur.$node.count();
					if(count > 1) txt += "(";
					txt = refs.reduce((txt,x) => {
						//const closed = cur.$closed.shift();
						//console.log(closed+"",x+"");
						return txt + x.serialize();
					},txt);
					if(count > 1) txt += ")";
					txt += ";\n}";
					console.log(txt);
