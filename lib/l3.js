...
class Close {
	...
	serialize() {
		...
		var ret;
		if(type == VAR) {
			if(node.count() > 1 && !ref.hasIfAssigned) {
				ret = ")";
		...
		return ret + (node.indexInParent < parentCount ? sep : "");
	}
}
	...
	serialize() {
		if(this.type == VAR) {
			if(this.isAssignment) {
				if(!this.hasIfAssigned) return "$" + this.name + " = " + (this.ref.length > 1 ? "replay(" : "") + "fromPromise(";
				return "";
			} else {
				return (this.isIfAssigned ? ("$" +this.isIfAssigned + " = ") :
					this.isReturn ? "return " : "") + "$" + this.name;
			}
		...
		if(type == EXPORT) {
			...
			const getDataType = (annot) => {
				...
				return {
					...
					occurs: hasOccurs ? getOccurs(annot) : 1,
				...
				if(v.type == VAR) {
					if(v.node.count() == 1) {
						if(typeof v.name == "number") {
							...
							const annot = typeseq.ref[v.name - 1];
							v.annot = annot;
							const {dataType, occurs, dataTypeRef} = getDataType(annot);
							...
							case "map": {
								...
								const {keyDataType,keyOccurs} = getDataType(keyTypeRef);
								if(keyOccurs != 1) throw new Error("Map key has wrong cardinality");
								const {valDataType,valOccurs} = getDataType(valTypeRef);
								if(valOccurs != 1) throw new Error("Map value has wrong cardinality");
								annot.dataType = _type.map;
								break;
							}
							case "array": {
								const valTypeRef = nodeMap.get(dataTypeRef.node.last());
								...
								annot.dataType = _type.array;

								break;
							}
							default:
								annot.dataType = _type[dataType];
							}
							console.log("annot",annot.dataType,dataType,occurs);
							v.external = true;
		...
		if (type == 17) {
			...
			if(node.node == cur.node) {
				if(cur.type == QUOT) {
					...
					if(last.type == EXPORT) {
						last.analyze(nodeMap);
						cur.analyze(nodeMap);
						ret.text += last.serialize();
					}
				} else if(cur.type == MODULE) {
					ret.text += cur.serialize();
				} else if(curQuot) {
