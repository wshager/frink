 ...
 */
function toL3($o) {
	return _seq.create(function (sink) {
		$o.subscribe({
			next: function next(node) {
				...
				case 14:
					...
					if (type == 1) {
						...
						try {
							for (var _iterator3 = node.attrEntries()[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
		...
		if (isClose) {
			...
			if (_isBranch) {
				...
				case 1:
				{
					...
					var _node = new _vnode.VNode(cx, _inode, _inode.$type, name, key, isLeaf(type) ? _inode.valueOf() : null, parent, _depth);
					// buffer attributes
					buffered = _node;
			...
			var node = new _vnode.VNode(cx, _inode, type, name, key, isLeaf(type) ? _inode.valueOf() : null, parent, depth);
			if (_isBranch) {
				parents.push(node);
			}
