...
function fromL3Stream(o) {
	...
	var checkStack = function checkStack() {
		...
		if (type == 18) {
			...
			var node = new _vnode.VNode(cx, _inode, type, name, key, isLeaf(type) ? _inode.valueOf() : null, parent, depth, parent ? parent.count() : 0);
			if (_isBranch) {
				parents.push(node);
			}
...
function toJS(o) {
	return o.reduce(function (ret, node) {
		...
		if (type != 17) {
			if (type == 18) {
				...
				case 14:
					{
						...
						let isAss = node.name && /^\$$/.test(node.name);
						if (isAss) {
							...
							if(!isDecl) {
								...
								} else if(ret.seqs[depth - 1]) {
									console.log("open seq",depth-1,ret.text);
							...
							if(ret.seqs[depth - 1] && ret.seqs[depth - 1]["$"]) {
								...
								if(!ifCases || ifCases != 1) ret.text += "return ";
							...
							if(type == 1) {
								...
								if(!ret.seqs[depth]) {
									ret.seqs[depth] = {};
									console.log("open seq",depth,ret.text);
								} else {
									// TODO what?
								}
				...
				case 15:
					{
						let depth = node.depth;
						ret.quotes[depth] = {};
						console.log("open quote",depth,ret.text);
						//let qname = isTuple ? stack[2] : stack[1];
						//qname = /[$:]/g.test(qname) ? qname.replace(/:/, ".") : qname;
						let ifCases = ret.ifCases[depth - 1];
						if(ifCases) {
							console.log("found ifCases for new quote",ifCases);
							//ret.text = ret.text.replace(/,$/, "");
							if(ifCases == 2) {
								ret.text += " else {";
							} else {
				...
				if (type == 8) {
					...
					//console.log("ifCasesV",depth,ret.ifCases[depth - 3]);
					let vars = ret.seqs[depth - 2] || ret.quotes[depth - 2] || ret.vars[depth - 1];
					console.log(value,vars);
					if(vars && node.indexInParent == 1 && vars["$"]) {
						...
						if(type != 12 && node.parent.count() > 1) {
							...
							if(!isVar && ret.quotes[depth - 2]) {
								ret.text += "return ";
								ret.quotes[depth - 2]["$"] = false;
								ret.quotes[depth - 2]["."] = true;
							}
							ret.text += type == 12 ? "a[" +(value - 1)+ "]" : "$" + value;
							if(node.parent && node.parent.parent && node.parent.parent.count() > node.parent.indexInParent) ret.text += ",";
						}
					} else {
						if(ret.quotes[depth - 1]) {
							ret.text += "return ";
			...
			if(ret.seqs[depth]) {
				...
				ret.seqs[depth] = null;
				if(ifCases) {
					console.log("found ifCases in seq",ifCases);
					//ret.text += ", test => { if (test) ";
				}
			} else if(ret.quotes[depth]) {
				console.log("close quote",depth);
				console.log(ret.quotes[depth]);
				if(ifCases) {
					console.log("found ifCases in quote",ifCases);
					ret.text += "}";
				} else {
					ret.text += "})";
				}
				...
				console.log("found ifCases",ifCases);
				...
				if(ret.seqs[depth - 1] && ret.seqs[depth - 1]["$"]) {
					...
					console.log("normal close when isModule");
					ret.text += ");\n";
				} else {
					if(ret.quotes[depth - 1] && ret.quotes[depth - 1]["."]) {
						console.log("quote already closed");
						ret.text += ";";
					} else {
		...
		return ret;
	}, { text: "", isDecl: false, isModule: false, modules: {}, quotes:[], ifCases:[], seqs: [], vars:[] }).map(function (ret) {
		ret.text = ret.text.replace(/,$/, "");
		delete ret.isDecl;
		return ret;
	});
