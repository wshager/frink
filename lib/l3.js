...
function _printTail(ret) {
	return JSON.stringify(ret.text.substr(-10));
}
	...
	return o.reduce(function (ret, node) {
		...
		if (type != 17) {
				...
				case 15:
					{
						...
						let ifCases = ret.ifCases[depth - 1];
						if(ifCases) {
							console.log("found ifCases for new quote",ifCases,_printTail(ret),depth,ret.quotes[depth - 3]);
							//ret.text = ret.text.replace(/,$/, "");
							if(ifCases == 1) {
								let vars = ret.quotes[depth - 3];
								let last = vars ? vars["$last"] : null;
								ret.text += ifCases == 1 ? "), ("+ (last ? "$" + last + "," : "") +"test) => { if (test) {" : "}";
							} else if(ifCases == 2) {
								ret.text += " else {";
							}
							ret.ifCases[depth - 1]++;
				...
				if (type == 8) {
					...
					// need to detect let-ret-seq always...
						...
						if(vars && node.indexInParent == 1 && vars["$"]) {
							...
							//console.log("found ifCases",depth - 2);
							//ret.text += "return ";
							}
							if(type == 3) {
								ret.text += "\"" + value.replace(/\\/g,"\\\\").replace(/"/g,"\\\"") + "\"";
							} else {
								ret.text += value;
							}
			...
			if(ret.seqs[depth]) {
				...
				if(count  > index && !ifCases) ret.text += ",";
				ret.vars[depth] = null;
				...
				if(ret.seqs[depth - 1] && ret.seqs[depth - 1]["$"]) {
					...
					if(ret.quotes[depth - 1] && ret.quotes[depth - 1]["."]) {
						...
						console.log("normal close when isModule",node.node.name);
