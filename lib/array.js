...
function array(...a) {
  ...
  if (l == 1) {
    a = a[0];
    return (0, _seq.isSeq)(a) ? a.reduce((acc, a) => acc.push(a), rrb.empty) : rrb.empty.push(a);
  }
...
function join($a) {
  ...
  return (0, _seq.foldLeft)((pre, cur) => {
...
function subarray(a, s, l) {
  s = s.valueOf() - 1;
  return (0, _util.isUndef)(l) ? rrb.slice(a, s) : l => rrb.slice(a, s, Math.max(s + Number(l), 0));
}
