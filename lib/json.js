...
function process(entry, parent, key) {
	...
	if (cc === Object) {
		if (entry.$children) {
			let name = entry.$name;
			let node = _persist.emptyINode(1, name, _persist.emptyAttrMap(entry.$attrs));
			parent = _persist.push(parent, [name, node]);
			for (var i = 0, l = entry.$children.length; i < l; i++) {
				process(entry.$children[i], node, i);
			}
			node = _persist.finalize(node);
		} else {
			let name = key !== undefined ? key : "#object";
			let node = _persist.emptyINode(6, name);
			parent = _persist.push(parent, [name, node]);
		...
		let node = _persist.emptyINode(5, name);
		parent = _persist.push(parent, [name, node]);
		...
		let node = _persist.ivalue(entry.constructor === String ? 3 : 12, key, entry);
		parent = _persist.push(parent, [key, node]);
	}
}
...
function toJS(doc) {
	function process(node, out, key) {
		...
		if (type == 1) {
			...
			for (let n of _access.children(node)) {
	...
	// discard DOC for now
	return process(_construct.ensureRoot(doc));
}
...
function fromL3(l3) {
	...
	const process = function (entry) {
		...
		if (type == 15) {
			...
			var val;
			if (type == 3) {
				val = _l.array2str(entry, index);
			} else if (type == 12) {
				val = _l.convert(_l.array2str(entry, index));
			} else if (type == 5) {
				val = [];
				parents[depth] = val;
			} else if (type == 6) {
