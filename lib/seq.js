...
LazySeq.prototype.toObservable = function () {
	...
	return _transducers.isObservable(iterable) ? iterable : _Observable.Observable.from(iterable);
};
...
LazySeq.prototype.toArray = function () {
	...
	if (_transducers.isObservable(iterable)) return iterable.reduce((a, x) => {
		a.push(x);
		return a;
	}, []);
...
LazySeq.prototype.transform = function (xf) {
	return new LazySeq(_transducers.transform(this.iterable, xf));
};
