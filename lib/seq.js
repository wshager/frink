...
function SeqIterator(iterable) {
	this.iter = iterable.next ? iterable : iterable[Symbol.iterator]();
}
...
function _isIterator(a){
	 return !!a && a.constructor.name.match(/iterator/i);
}

function seq(...a) {
	if (a.length == 1){
		if(isSeq(a[0])) return a[0];
		if(_isArray(a[0])) return new LazySeq(a[0]);
		//return new LazySeq(Array.from(a[0]));
	}
	var s = new LazySeq();
