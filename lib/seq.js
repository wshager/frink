...
_Observable.Observable.prototype.call = function($, ...a) {
	//this.count().subscribe(x => x > 1 ? console.log("TOOBIG",a[0],x) : null);
...
var forEach = exports.forEach = function forEach($s, $fn) {
	if (!(0, _util.isUndef)($fn)) {
		if (!isSeq($s)) {
			//console.log("forEach direct complete",$fn($s));
			return $fn($s);
			//$s = of($s);
		}
...
function seq() {
	...
	return from(a.map(x => isSeq(x) ? x : _Observable.Observable.of(x))).concatAll();
}
...
function exactlyOne($arg) {
	...
	return isExactlyOne(s).switchMap(function(test) {
		if(!test) $arg.subscribe(x => require("./console").log("exactlyOne failed",x));
