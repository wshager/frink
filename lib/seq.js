...
exports.isExactlyOne = exports.isOneOrMore = exports.isZeroOrOne = exports.first = exports.skip = exports.take = exports.fromPromise = exports.of = exports.from = exports.foldLeft = exports.filter = exports.forEach = undefined;
...
var fromArgs = function fromArgs(args) {
...
function compose() {
	...
	return asArray(fromArgs(args)).map(function (a) {
...
var foldLeft = exports.foldLeft = function foldLeft() {
	...
	var $s = seq(arguments[0]);
	//	var $fn = exactlyOne(len == 2 ? arguments.length <= 1 ? undefined : arguments[1] : arguments.length <= 2 ? undefined : arguments[2]);
	var $seed = seq(arguments[1]);
	var $fn = seq(arguments[2]);
	var _reducer = function _reducer($s,$fn,$seed) {
		return $fn.concatMap(function (fn) {
			return $s.reduce(_wrap(fn), $seed).concatAll();
		});
	};
	return _reducer($s,$fn,$seed);//.concatAll();
};
...
function seq() {
	...
	if (a.length == 1) {
		...
		if (Array.isArray(x) || x[Symbol.iterator] && typeof x != "string" && !(0, _util.isUntypedAtomic)(x) && !(0, _access.isVNode)(x) && !(0, _util.isList)(x) && !(0, _util.isMap)(x)) return from(x);
		return of(x);
	}
