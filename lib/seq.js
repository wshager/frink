...
function callify($s) {
	const f = function(...a) {
		return forEach(exactlyOne($s),s => {
			const _isList = _util.isList(s);
			if(_isList || _util.isMap(s)) {
				const len = a.length;
				let [k,v] = a;
				if(len == 1) return forEach(exactlyOne(k), k => s.get(_isList? k - 1 : k));
...
module.exports.callify = callify;
...
var forEach = exports.forEach = function forEach($s, $fn,n) {
	if (!(0, _util.isUndef)($fn)) {
		if(!isSeq($s)) {
			return $fn($s);
		}
		//return seq($fn).switchMap(fn => {
		return callify(create($o => {
			var index = 0;
			var active = 1;
			$s.subscribe({
				next:function (x) {
					...
					if(isSeq(ret)) {
						ret.subscribe({
							...
							complete:() => {
								active--;
								//console.log("forEach inner complete",name,active);
				...
				complete:() => {
					active--;
					//console.log("forEach outer complete",name,active);
					if(active == 0) $o.complete();
				},
				error:err => {
					$o.error(err);
...
var foldLeft = exports.foldLeft = function foldLeft($s,$seed,$fn) {
	let $out = $s.reduce(function(a,x){
		let $t = $fn(a,x);
		return isSeq($t) ? $t : of($t);
	}, $seed).concatAll();
	return callify($out);
};

var from = exports.from = function from(a) {
	return callify(_Observable.Observable.from(a));
};

var of = exports.of = function of(a) {
	return callify(_Observable.Observable.of(a));
};

var fromPromise = exports.fromPromise = function fromPromise(a) {
	return callify(_Observable.Observable.fromPromise(a));
};
...
function seq() {
	...
	if (a.length == 0) return callify(_Observable.Observable.empty());
	if (a.length == 1) {
		var x = a[0];
		if (isSeq(x)) {
			return callify(x);
		}
		if ((0, _util.isUndefOrNull)(x)) return callify(_Observable.Observable.empty());
		if ((0, _util.isObject)(x) && (x instanceof Promise || typeof x.then == "function")) return fromPromise(x);
		if (Array.isArray(x) || x[Symbol.iterator] && typeof x != "string" && !(0, _util.isDOMNode)(x) && !(0, _util.isUntypedAtomic)(x) && !(0, _access.isVNode)(x) && !(0, _util.isList)(x) && !(0, _util.isMap)(x)) return callify(from(x));
		return of(x);
	}
	const out = create($o => {
		var len = a.length;
		a.forEach(s => {
			if(isSeq(s)) {
				...
				s.subscribe({
					...
					complete: () => {
						len--;
						//console.log("completeseq",id,len);
						if(!len) $o.complete();
					}
				});
			} else {
				//console.log("combine",s);
				$o.next(s);
				len--;
...
function create(o) {
	return callify(_Observable.Observable.create(o));
}
...
function range($n) {
	...
	return $s.concatMap(function (s) {
		return $n.concatMap(function (n) {
			return callify(_Observable.Observable.range(s, n));
		});
	});
}
...
function exactlyOne($arg) {
	...
	return isExactlyOne(s).switchMap(function (test) {
		if(!test) $arg.subscribe(x => console.log("exactlyOne failed",x));
