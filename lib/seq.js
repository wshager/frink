...
class ArraySeq {
	...
	mergeMap(fn) {
		...
		return hasOb ? _Observable.Observable.from(buffer).concatAll() : new ArraySeq(buffer);
	}
	concatMap(fn) {
...
exports.replay = x => isSeq(x) ? x.shareReplay() : x;

exports.boolean = boolean;

exports.booleanPromise = x => toPromise(boolean(x));
...
exports.foldLeft = function foldLeft($a, $seed, $fn) {
	...
	return $a.reduce(function(acc,x){
		...
		return fromPromise(ret);
	},$seed).mergeAll().shareReplay(1);
};

exports.scan = function scan($a,$seed,$fn) {
	if(!isSeq($a)) {
		const ret = $fn($seed,$a);
		return fromPromise(ret);
	}
	return $a.scan(function(acc,x){
		const ret = $fn(acc,x);
		//if(name) console.log("SEQ",name,acc,x,ret);
		return fromPromise(ret);
...
var fromPromise = exports.fromPromise = function fromPromise(a) {
	if(a instanceof Promise) return create(subscribeToPromise(a),queue);
	return a;
};
...
function seq() {
	...
	return new ArraySeq(a).concatAll();
}
...
function exactlyOne($arg,name) {
	...
	return isExactlyOne(s).switchMap(function(test) {
		if(!test) require("./console").log("exactlyOne failed",$arg,name);
