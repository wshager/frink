...
function callify(s) {
	const f = function(...a) {
		if(f.__callable) return f.__callable.apply(f,a);
		throw new Error("No callable provided");
	};
	f.__proto__ = s;
	return f;
}
...
var foldLeft = exports.foldLeft = function foldLeft() {
	...
	var $seed = len == 2 ? undefined : arguments.length <= 1 ? undefined : arguments[1];
	var $fn = len == 2 ? arguments.length <= 1 ? undefined : arguments[1] : arguments.length <= 2 ? undefined : arguments[2];//exactlyOne();
	//return exactlyOne($fn).concatMap(function (fn) {
	return $s.reduce($fn, $seed).concatAll();
	//}).concatAll();
};
...
function isSeq(a) {
	return !!(a && (a instanceof _Observable.Observable || a.__is_ArraySeq));
}
...
function seq() {
	...
	if (a.length == 1) {
		var x = a[0];
		if (isSeq(x)) return x.__callable ? x : callify(x);
	...
	const out = from(a).concatMap(function (a) {
		return seq(a);
	});
	return callify(out);
}
