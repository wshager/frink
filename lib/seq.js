...
exports.isExactlyOne = exports.isOneOrMore = exports.isZeroOrOne = exports.first = exports.skip = exports.take = exports.compose = exports.fromPromise = exports.of = exports.from = exports.foldLeft = exports.filter = exports.forEach = undefined;
...
Object.defineProperty(exports, "compose", {
	enumerable: true,
	get: function get() {
		return (...args) => seq(args.map(x => {
			return seq(x);
		})).concatAll().reduce((a,x) => {
			console.log(x);
			a.push(x);
			return a;
		},[]).map(a => _pipe.pipe.apply(a,a));
	}
});
...
var forEach = exports.forEach = function forEach($s, $fn) {
	...
	return seq($s).map(function (fn) {
		return _operators.concatMap(function (x) {
			return fn(x);
		});
	});
};
...
function transform($arg, $fn, $init) {
	var s = seq($arg);
	return seq($fn).concatMap(function (fn) {
		...
		return isExactlyOne(s).switchMap(function (test) {
			return test ? s.concatMap(function (x) {
				...
				}, !(0, _util.isUndef)($init) ? $init : x["@@transducer/init"]()).map(function (x) {
