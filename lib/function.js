...
function functionLookup($name, $arity) {
    ...
    var uri = _seq.first(qname._uri).toString();
    var name = camelCase(_seq.first(qname._name).toString().split(/:/).pop());
...
function apply($fn, $a) {
    ...
    if (!(a instanceof Array)) {
        if (typeof a.toArray != "function") return _error.error("");
    }
    return _seq.first($fn).apply(this, a.toArray());
}
