...
dawg.backtrack = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.backtrack", $len);
	if ($len == 2) return n.fromPromise(dawg.backtrack$2.apply(null, $));
};
dawg.traverse = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.traverse", $len);
...
dawg.loop = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.loop", $len);
	if ($len == 6) return n.fromPromise(dawg.loop$6.apply(null, $));
};
dawg.matchPos = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.matchPos", $len);
	if ($len == 3) return n.fromPromise(dawg.matchPos$3.apply(null, $));
};
dawg.matchWord = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.matchWord", $len);
	if ($len == 2) return n.fromPromise(dawg.matchWord$2.apply(null, $));
};
dawg.find = (...$) => {
	const $len = $.length;
	if (process.env.debug) console.log("dawg.find", $len);
