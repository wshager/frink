...
dawg.backtrack = (...$) => {
	const $len = $.length;
	global.console.log("dawg.backtrack", $len);
	if ($len == 2) return n.fromPromise(dawg.backtrack$2.apply(null, $));
};
dawg.traverse = (...$) => {
	const $len = $.length;
	global.console.log("dawg.traverse", $len);
