...
dawg.backtrack$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), (async function(...$) {
	...
	return $("path", $(1)),
		$("b", $(2)),
		await n.when(n.gt(array.size($("path")), 0)) ? ($("entry", array.head($("path"))),
			await n.when(n.eq($("entry").call($, "_k"), $("b"))) ? ($("entry").call($, "_v")) : (dawg.backtrack(array.tail($("path")), $("b")))) : (n.seq())
}));
...
dawg.traverse$4 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item()), n.item()), (async function(...$) {
	...
	return $("ret", $(1)),
		...
		$("b", $(3)),
		$("path", $(4)),
		await n.when(n.or(n.empty($("ret")), n.seq(n.and(n.instanceOf($("ret"), n.array(n.occurs(n.item(), n.zeroOrMore()))), n.eq(array.size($("ret")), 0))))) ?
			(n.seq()) :
			...
			dawg.traverse(
				...
				n.tail($("word")), $("b"), $("path"))) :
					...
					(await n.when(n.gt(array.size($("ret")), 0)) ?
						...
						(n.seq())) : ($("ret"))),
				await n.when(
					n.and(n.instanceOf($("ret"), map.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore()))),
						n.eq($("ret").call($, "_k"), $("b")))) ?
							($("ret").call($, "_v")) :
							($("entry", dawg.backtrack($("path"), $("b"))),
					await n.when(n.boolean($("entry"))) ? ($("entry")) : (array.array($("ret"), $("path"))))))
}));
dawg.loop$6 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item(), n.item(), n.item()), n.item()), (async function(...$) {
	...
	return $("entry", $(1)),
		...
		$("pos", $(5)),
		$("path", $(6)),
		await n.when(n.gt(array.size($("entry")), 0)) ? ($("a", array.head($("entry"))),
			$("isEntry", map.contains($("a"), "_v")),
			await n.when(n.boolean($("isEntry"))) ? ($("has", await n.when(n.boolean($("isEntry"))) ? (console.log("cp3",$("cp")),dawg.matchPos($("a"), $("pos"), $("cp"))) : (n.false())),
				$("path", await n.when(n.boolean($("has"))) ? ($("len", array.size($("path"))),
					await n.when(n.or(n.eq($("len"), 0), n.ne($("path").call($, $("len")).call($, "_v"), $("a").call($, "_v")))) ? (array.append($("path"), $("a"))) : ($("path"))) : ($("path"))),
				$("ret", await n.when(n.boolean($("has"))) ? ($("a")) : (array.filter($("path"), n.typed(n.function(n.seq(n.item()), n.item()), (async function(...$) {
					$ = n.frame($);
					return $("entry", $(1)),
						console.log("cp4",$("cp")),
						dawg.matchPos($("entry"), $("pos"), $("cp"))
				}))))),
				dawg.loop(array.tail($("entry")), $("ret"), $("cp"), $("word"), $("pos"), $("path"))) : (await n.when(map.contains($("a"), $("cp"))) ?
				(array.array($("a").call($, $("cp")), $("path"))) :
				(dawg.loop(array.tail($("entry")), $("ret"), $("cp"), $("word"), $("pos"), $("path"))))) : (array.array($("ret"), $("path")))
}));
...
dawg.find$4 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item()), n.item()), (async function(...$) {
	...
	return $("entry", $(1)),
		...
		(await n.when(dawg.matchPos($("entry"), $("pos"), $("cp"))) ? ($("len", array.size($("path"))),
			$("path", await n.when(n.or(n.eq($("len"), 0), n.ne($("path").call($, $("len")).call($, "_v"), $("entry").call($, "_v")))) ?
			(array.append($("path"), $("entry"))) : ($("path"))),
			array.array($("entry"), $("path"))) : (array.array(array.filter($("path"), n.typed(n.function(n.seq(n.item()), n.item()), (async function($$,...$) {
			$ = n.frame($,$$);
			return $("entry", $(1)),
				console.log("cp2"),
				dawg.matchPos($("entry"), $("pos"), $("cp"))
		}).bind(null,$))), array.array()))) : (array.array($("entry").call($, $("cp")), $("path"))))
}));
dawg.backtrack = (...$) => {
	const $len = $.length;
	console.log("backtrack",$len)
	if ($len == 2) return n.fromPromise(dawg.backtrack$2.apply(null, $));
};
dawg.traverse = (...$) => {
	const $len = $.length;
	console.log("traverse",$len)
	if ($len == 2) return n.fromPromise(dawg.traverse$2.apply(null, $));
	if ($len == 4) return n.fromPromise(dawg.traverse$4.apply(null, $));
};
dawg.loop = (...$) => {
	const $len = $.length;
	console.log("loop",$len)
	if ($len == 6) return n.fromPromise(dawg.loop$6.apply(null, $));
};
dawg.matchPos = (...$) => {
	const $len = $.length;
	console.log("matchPos",$len)
	if ($len == 3) return n.fromPromise(dawg.matchPos$3.apply(null, $));
};
dawg.matchWord = (...$) => {
	const $len = $.length;
	console.log("matchWord",$len)
	if ($len == 2) return n.fromPromise(dawg.matchWord$2.apply(null, $));
};
dawg.find = (...$) => {
	const $len = $.length;
	console.log("find",$len)
