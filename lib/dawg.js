...
dawg.traverse$4 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item()), n.item()), (async function(...$) {
	...
	return $("ret", $(1)),
		...
		$("b", $(3)),
		$("path", $(4)),
		await n.when(await n.when(n.empty($("ret"))) || await n.when(n.seq(await n.when(n.instanceOf($("ret"), n.array(n.occurs(n.item(), n.zeroOrMore())))) && await n.when(n.eq(array.size($("ret")), 0))))) ?
		(n.seq()) :
		(await n.when(n.exists($("word"))) ?
			...
			($("ret", await n.when(n.instanceOf($("ret"), n.array(n.occurs(n.item(), n.zeroOrMore())))) ?
					(await n.when(n.gt(array.size($("ret")), 0)) ?
						...
						(n.seq())) :
					($("ret"))),
				await n.when(await n.when(n.instanceOf($("ret"), n.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore())))) && await n.when(n.eq($("ret").call($, "_k"), $("b")))) ?
				($("ret").call($, "_v")) :
		...
		($("a", array.head($("entry"))),
			...
			($("has", await n.when(n.boolean($("isEntry"))) ?
				...
				$("path", await n.when(n.boolean($("has"))) ?
					($("len", array.size($("path"))),
						await n.when(await n.when(n.eq($("len"), 0)) || await n.when(n.ne($("path").call($, $("len")).call($, "_v"), $("a").call($, "_v")))) ?
						(array.append($("path"), $("a"))) :
						($("path"))) :
					($("path"))),
	...
	return $("entry", $(1)),
		...
		(await n.when(map.contains($("entry"), "_v")) ?
			(await n.when(dawg.matchPos($("entry"), $("pos"), $("cp"))) ?
				($("len", array.size($("path"))),
					$("path", await n.when(await n.when(n.eq($("len"), 0)) || await n.when(n.ne($("path").call($, $("len")).call($, "_v"), $("entry").call($, "_v")))) ?
...
dawg.loop = (...$) => {
	const $len = $.length;
	//global.console.log("dawg.loop",$len)
