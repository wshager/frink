...
dawg.traverse$4 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item()), n.item()), ((...a) => {
	...
	let $b = a[2];
	let $path = a[3];
	return n.forEach(n.or(n.empty($ret), n.seq(n.and(n.instanceOf($ret, n.array(n.occurs(n.item(), n.zeroOrMore()))), n.eq(array.size($ret), "0")))), (($ret,$word,$b,$path,test) => {
		if (test) {
			return n.seq();
		} else {
			return n.forEach(n.exists($word), (($ret,$word,$b,$path,test) => {
				if (test) {
					let $c = n.head($word);
					$b = n.concat($b, $c);
					...
					$ret = n.forEach(n.instanceOf($ret, n.array(n.occurs(n.item(), n.zeroOrMore()))), test => {
						if (test) {
...
dawg.find$4 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item()), n.item()), ((...a) => {
	...
	return n.forEach(n.instanceOf($entry, array.array(n.occurs(n.item(), n.zeroOrMore()))), test => {
		if (test) {
			...
			return n.forEach(map.contains($entry, "_v"), test => {
				if (test) {
					...
					return array.array($entry($cp), $path);
				}
			});
		}
	});
