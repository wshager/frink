...
function iterator(coll) {
	...
	if (iter) {
		...
		return coll;
	} else if ((0, _util.isArray)(coll)) {
		return new ArrayIterator(coll);
	} else if ((0, _util.isObject)(coll)) {
...
function reduce(coll, xform, init) {
	...
	} else if ((0, _util.isObject)(coll) || fulfillsProtocol(coll, "iterator")) {
...
function merge(obj, x) {
	if ((0, _util.isArray)(x) && x.length === 2) {
		obj[x[0]] = x[1];
	} else {
...
function into(from, xform, to) {
	if ((0, _util.isArray)(to)) {
		return transduce(from, xform, arrayReducer, to);
	} else if ((0, _util.isObject)(to)) {
...
function foldLeft(iterable, z, f) {
	if ((0, _util.isFunction)(iterable.reduce)) {
