...
function vnode(inode, parent, depth, indexInParent, type) {
	...
	var name,
	    key = inode.$key,
	    value;
	//if (type != 2 && inode.$value) inode = inode.$value;
...
function push(inode, val, type) {
	type = type || _inferType(inode);
	if (type == 1 || type == 9 || type == 11) {
		inode.$children.push(val.inode);
	} else if (type == 5) {
		inode.push(val.inode);
	} else if (type == 6) {
		inode[val.$name] = val.$value;
	}
	return inode;
}
...
function stringify(inode, type, json) {
