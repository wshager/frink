...
function _elemToString(e) {
	...
	str = (0, _util.foldLeft)(Object.entries(e.$attrs), str, attrFunc);
...
function keys(inode, type) {
	...
	if (type == 5) return (0, _util.range)(inode.length).toArray();
...
function stringify(inode, type) {
	...
	if (type == 1) {
		...
		var val = (0, _util.forEach)(inode, function (c) {
			return stringify(c, 0, true, false);
		}).join("");
		str += "<l3:l" + (val ? ">" + val + "</l3:l>" : "/>");
	} else if (type == 6) {
		var _val = (0, _util.forEach)(Object.entries(inode), function (c) {
