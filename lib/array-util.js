...
a.firstIndexOf$2 = n.typed(n.function(n.seq(n.array(n.occurs(n.item(), n.zeroOrMore())), n.occurs(n.item(), n.zeroOrOne())), n.item()), (async function(...$) {
	...
	a.foldLeftAt($("array"), n.seq(), n.typed(n.function(n.seq(n.item(), n.item(), n.item()), n.item()), (async function(...$) {
		...
		$("cur", $(2)),
		$("at", $(3)),
		(await n.when(n.empty($("pre"))) || await n.when(n.deepEqual($("cur"), $("lookup")))) ?
			($("at")) :
			($("pre"));
	}).bind($)));
}));
a.lastIndexOf$2 = n.typed(n.function(n.seq(n.array(n.occurs(n.item(), n.zeroOrMore())), n.occurs(n.item(), n.zeroOrOne())), n.item()), (async function(...$) {
	...
	a.foldRightAt($("array"), 0, n.typed(n.function(n.seq(n.item(), n.item(), n.item()), n.item()), (async function(...$) {
		...
		$("pre", $(2)),
		$("at", $(3)),
		(await n.when(n.eq($("pre"), 0)) && await n.when(n.deepEqual($("cur"), $("lookup")))) ?
			($("at")) :
			($("pre"));
	}).bind($)));
}));
a.put = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.put", $len);
	if ($len == 3) return n.fromPromise(a.put$3.apply(null, $));
};
a.foldLeft = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.foldLeft", $len);
...
a.foldLeftAt = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.foldLeftAt", $len);
...
a.reduceAroundAt = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.reduceAroundAt", $len);
...
a.reduceAheadAt = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.reduceAheadAt", $len);
...
a.foldRight = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.foldRight", $len);
...
a.foldRightAt = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.foldRightAt", $len);
...
a.forEach = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.forEach", $len);
...
a.forEachAt = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.forEachAt", $len);
...
a.last = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.last", $len);
	if ($len == 1) return n.fromPromise(a.last$1.apply(null, $));
};
a.pop = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.pop", $len);
	if ($len == 1) return n.fromPromise(a.pop$1.apply(null, $));
};
a.firstIndexOf = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.firstIndexOf", $len);
	if ($len == 2) return n.fromPromise(a.firstIndexOf$2.apply(null, $));
};
a.lastIndexOf = (...$) => {
	const $len = $.length;
	if(process.env.debug) global.console.log("a.lastIndexOf", $len);
