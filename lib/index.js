...
exports.serialize = function serialize($s) {
	...
	// TODO fix 2 problems:
	// 1: parent must complete after items (ex. array /w maps must complete after all child maps)
	...
	const _serialize = ($o,$s,parent) => {
		const _next = (s) => {
			if(parent && parent.__key) {
				//console.log("found key",parent.__key)
			...
			if(_util.isList(s)) {
				...
				if(s.size) {
					...
					console.log("closing empty array")
					$o.next([17]);
				}
			...
			$s.subscribe({
				...
				complete: () => {
					...
					if(parent && resolved.has(parent)) {
						...
						if(size == parent.size) {
							console.log("closing parent",size,parent.toJS())
							$o.next([17]);
							resolved.delete(parent);
						}
					}
					//console.log("outer done",active);
					if(!active) {
						$o.complete();
					}
				},
