...
exports.serialize = function serialize($s,k) {
	// FIXME on complete, reassemble arrays/maps
	const _serialize = s => {
		const isArray = _util.isList(s);
		const isMap = !isArray && _util.isMap(s);
		s = isArray || isMap ? s.toJS() : s;
		if(isArray) {
			return _seq.from(s.map(x => _seq.seq(serialize(x,k)))).concatAll();
		} else if(isMap) {
			return _seq.from(Object.entries(s).map(([k,v]) => _seq.seq(serialize(v,k)))).concatAll();
		} else if(_seq.isSeq(s)) {
			console.log("SEEQQQ");
			return serialize(s,k);
		}
		if(k) {
			return {[k]:s};
		}
	...
	return _seq.forEach($s,_serialize);
};
