...
Value.prototype[Symbol.iterator] = /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
...
function _elemToString(e) {
	...
	if (e.size > 0) {
		...
		try {
			for (var _iterator = e.values()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
				var c = _step.value;
...
OrderedMap.prototype.toString = function () {
	...
	var objFunc = function objFunc(kv) {
		return "\"" + kv[0] + "\":" + kv[1].toString(false, true);
	};
	if (type == 1) {
		...
		str += "{";
		str += (0, _util.pipe)((0, _util.forEach)(objFunc), this).join(",");
		str += "}";
	} else if (type == 9) {
...
function vnode(inode, parent, depth, indexInParent) {
	return new _vnode.VNode(cx, inode, inode._type, inode._ns ? (0, _qname.q)(inode._ns.uri, inode._name) : inode._name, inode._value, parent, depth, indexInParent);
}
