...
function string($a) {
	...
	return _seq.isSeq($a) || _access.isVNode($a) ? data($a, String, emptyString) : cast($a, String, emptyString);
}
...
function cast($a, $b, emptyType = null) {
	if (_seq.isSeq($a)) {
...
function _boolean($a) {
	if (_seq.isSeq($a)) {
		...
		if (!s) return false;
		var a = _seq.first($a);
		var _isVNode = _access.isVNode(a);
		if (s > 1 && !_isVNode) {
...
function data($a, castToType = UntypedAtomic, emptyType = emptyUntypedAtomic) {
...
function dataImpl(node, fltr, castToType, emptyType) {
	...
	if (_seq.isSeq(node)) {
		...
		var a = _transducers.into(node, _transducers.compose(_transducers.forEach(_ => dataImpl(_, fltr, castToType, emptyType)), _transducers.filter(_ => undefined !== _)), _seq.seq());
		if (!a.size) {
			ret = emptyType();
		} else {
			ret = a;
	...
	if (type === 1) {
		ret = _transducers.into(node, _transducers.compose(_transducers.forEach(_ => dataImpl(_, fltr, castToType, emptyType)), _transducers.filter(_ => undefined !== _)), _seq.seq());
	} else {
		ret = node.value;
	...
	return _seq.isSeq(ret) && !ret.size ? emptyType() : ret;
}

function instanceOf($a, $b, card = null) {
