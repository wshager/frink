			...
			var _handle = (a) => {
				...
				if(bIsSeq) {
					return $b.subscribe({
						next: function(b){
							var ret = opfn(a,b);
							//console.log("b",a,b,ret,operator);
							complete(ret);
							//this.unsubscribe();
...
function not($a) {
	return _seq.forEach(boolean($a),function (a) {
		return !a;
	});
}
