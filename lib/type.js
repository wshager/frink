...
var ReplaySubject = require("rxjs/ReplaySubject").ReplaySubject;
...
function decimal($a) {
	if(_util.isUndef($a)) return decimal;
	// type test
	return _seq.forEach($a,function (a) {
		return cast(a, _big2.default, zeroInt);
	});
}

function integer($a) {
	if(_util.isUndef($a)) return integer;
	return _seq.forEach($a,function (a) {
...
function string($a) {
	...
	// type test
	var cc = String;
	if ((0, _util.isUndef)($a)) return cc;
...
function number($a) {
	...
	var cc = Number;
	if ((0, _util.isUndef)($a)) return cc;
	return _seq.forEach($a,_seq.zeroOrOne($a),function (a) {
...
function cast($a, $b) {
	...
	return _seq.forEach((0, _seq.empty)($a),function (test) {
...
function indexOf($a, $b) {
	return _seq.forEach((0, _seq.zeroOrOne)($b),function (b) {
		return _seq.seq($a).map(function (x, i) {
			return b === x ? i + 1 : 0;
		}).filter(function (x) {
			return x;
		});
...
function _opReducer($a, opfn, $b, general,operator) {
	const aIsSeq = _seq.isSeq($a);
	const bIsSeq = _seq.isSeq($b);
	if (general) {
		//console.trace($a,$b);
		if(!aIsSeq && !bIsSeq) {
			return opfn($a,$b);
		}
		/*if(aIsSeq && $a.__is_ArraySeq) {
			if(bIsSeq) {
				if($b.__is_ArraySeq) {
					let ret = false;
					$a.takeWhile(a => {
						for(const b of $b.$array) {
							ret = opfn(a,b);
							if(ret) return !ret;
						}
						return !ret;
					});
					//console.log(operator,$a,$b,ret);
					return ret;
				} else {
					$a = $a.toObservable();
				}
			...
			if(bIsSeq && $b.__is_ArraySeq) $b = $b.toObservable();
		}*/
		return _seq.exists(_seq.filter($a,a => _seq.filter($b, b => opfn(a,b))));
		//console.log("gen",$a,$b);
		return _seq.create($o => {
			...
			const _next = a => {
				//console.log(operator,"a",a,$b);
				if(bIsSeq) {
					active++;
					$b.takeWhile(() => !val).subscribe({
						next: b => {
							val = opfn(a,b);
							//console.log(operator,a,b,val);
							if(val) {
								complete();
							}
						},
		...
		//$a = _seq.zeroOrOne($a);
		//$b = _seq.zeroOrOne($b);
		//console.trace(operator,$a,$b);
		return _seq.forEach($a, a => _seq.forEach($b, b => opfn(a,b)));
	}
}

function not($a) {
	const ret = _seq.boolean($a);
	return _seq.isSeq(ret) ? _seq.forEach(ret, a => !a) : !ret;
}
...
// and the result to r
// when the original is applied, this will simply return the input if valid, or nothing if invalid
...
const func = (s,r) => fn => {
	const f = (...args) => {
		const ret = fn.apply(null,args.map((a,i) => {
			const t = s[i](a);
			if(typeof t == "function" && t.hasOwnProperty("__wraps")) {
				if(t.__wraps !== a) throw new Error("unknown function found");
			} else {
				if(t !== a) throw new Error("incorrect type");
			}
			return t;
		}));
		if(ret instanceof ReplaySubject) {
			ret.subscribe(ret => {
				if(r(ret) !== ret) throw new Error("incorrect output");
			});
		} else {
			if(r(ret) !== ret) throw new Error("incorrect output");
		}
...
/*
var array = (a) => Array.isArray(a) ? a : void 0;
var add = (a,b) => a + b;
var addOne = add.bind(null,1);
var map = (a,f) => a.map(x => f(x));
func([array,func([Number],Number)],array)(map)(["1",2,3],addOne);
*/
const isAtomic = (a,t) => (t = t || a.constructor,t === Number || t === String || t === Boolean /*|| t === Decimal*/ || t === Integer);
exports.occurs = a => a;
exports.array = a => !_array.isArray(a) || a;
exports.map = a => !_map.isMap(a) || a;
exports.atomic = a => !isAtomic(a) || a;
exports.item = () => a => a;
exports.typed = (t,a) => t(a);
