...
function string($a) {
	...
	return card($a).concatMap(function (a) {
...
function number($a) {
	...
	var cc = Number;
	if ((0, _util.isUndef)($a)) return (0, _seq.seq)(cc);
	return (0, _seq.zeroOrOne)($a).map(function (a) {
...
function boolean($a) {
	...
	return (0, _seq.empty)($a).concatMap(function (test) {
		return test ? (0, _seq.seq)(false) : hasZeroOrOne($a).concatMap(function (test) {
			return test ? $a.map(function (a) {
				return a.valueOf();
			}) : $a.concatMap(function (a, i) {
				return (0, _access.isVNode)(a) ? true : (0, _error.error)("err:FORG0006", "Item " + (i + 1) + " is not a node");
			});
		});
	});
}
...
function and($a, $b) {
	$a = boolean($a);
	$b = boolean($b);
	return _opReducer($a, function (a, b) {
		return a && b;
	}, $b);
}
...
function data($a) {
	...
	return (0, _seq.seq)($a).concatMap(a => dataImpl(a, castToType, emptyType));
}
...
function dataImpl(node, castToType, emptyType) {
	...
	console.log(node);
	return _seq.seqFrom(_access.vdoc(node)).concatMap(node => {
		console.log(node);
		var type = node.type;
		var $ret;
		if (type == 3) {
		...
		return (0, _seq.empty)($ret).concatMap(function (isEmpty) {
			return isEmpty ? (0, _seq.seq)(emptyType()) : $ret;
		});
	});
}
