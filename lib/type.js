...
function integer($a) {
    return cast(_transducers.forEach($a, Math.floor), Integer, zeroInt);
}

function string($a) {
    // type test
    return _seq.isSeq($a) || _access.isNode($a) ? data($a) : cast($a, String, emptyString);
}
...
function call(...a) {
    ...
    let args = a.slice(1);
    if (_array.isArray(f)) {
        return f.get(_seq.first(a[1]) - 1);
    } else if (_map.isMap(f)) {
        var key = _seq.first(a[1]);
        return f.get(key);
    } else {
...
function _boolean($a) {
    var a = _seq.first($a);
    if (_seq.isEmpty(a)) return false;
    if ($a.size > 1 && !_access.isNode(a)) {
        throw _error.error("err:FORG0006");
    }
    return a.valueOf();
}
...
function op($a, operator, $b) {
    ...
    if (typeof operator == "string") {
        ...
        if (!general) {
            if (_seq.isEmpty($a)) return $a;
            if (_seq.isEmpty($b)) return $b;
...
function dataImpl(node, fltr = false) {
    var ret;
    if (_seq.isSeq(node)) {
        if (_seq.isEmpty(node)) return node;
        //ret = node.map(_ => dataImpl(_, fltr)).filter(_ => _ !== undefined);
        var a = _transducers.transform(node, _transducers.compose(_transducers.forEach(_ => dataImpl(_, fltr)), _transducers.filter(_ => undefined !== _)));
        if (!a.size) {
            ret = _seq.seq(new UntypedAtomic(""));
        } else {
            ret = a;
        }
        return ret;
    ...
    if (!_access.isNode(node)) return node;
    var type = node.type;
