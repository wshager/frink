...
function string($a) {
	...
	return _seq.isSeq($a) || _access.isVNode($a) ? data($a,String) : cast($a, String, emptyString);
}
...
function cast($a, $b, emptyType) {
	if (_seq.isSeq($a)) {
		if(!$a.size) return emptyType();
		return op($a, _cast, $b);
	}
	return _cast($a, $b, emptyType);
}

function _cast(a, b, emptyType) {
	if(a === undefined || a === null) return emptyType();
...
function data($a,castToType,emptyType) {
	return dataImpl($a,false,castToType,emptyType);
}

function dataImpl(node, fltr = false, castToType = UntypedAtomic,emptyType=emptyUntypedAtomic) {
	var ret;
	if (_seq.isSeq(node)) {
	...
	return _seq.isSeq(ret) && !ret.size ? new emptyType() : ret;
}
