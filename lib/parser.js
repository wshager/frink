...
class Parser extends _events.EventEmitter {
	...
	reset() {
		var last = _vnode.emptyINode(9, "#document", _vnode.emptyAttrMap());
		var parents = [];
		...
		saxParser.onopentag = function (node) {
			...
			let n = _vnode.emptyINode(nodeType, nodeName, _vnode.emptyAttrMap(attrs));
			if (last) {
				last = _vnode.push(last, [nodeName, n]);
				parents.push(last);
			}
			last = n;
		}.bind(this);
		saxParser.onclosetag = function () {
			...
			last = _vnode.finalize(last);
			last = parents.pop();
		};
		saxParser.onend = function () {
			saxParser.ended = true;
			return this.emit("end", _vnode.finalize(last));
		}.bind(this);
		var ontext = function (val, type = 3) {
			if (/\S/.test(val)) {
				let name = _vnode.count(last) + 1;
				let n = _vnode.value(type, name, val);
				last = _vnode.push(last, [name, n]);
			}
		};
		...
		saxParser.ondoctype = function (value) {
			last = _vnode.setAttribute(last, "DOCTYPE", value);
		};
		saxParser.onprocessinginstruction = function (pi) {
			last = _vnode.setAttribute(last, pi.name, pi.body);
		};
