...
class Parser extends _events.EventEmitter {
	...
	reset() {
		var last = _pvnode.emptyINode(9, "#document", _pvnode.emptyAttrMap());
		var parents = [];
		...
		saxParser.onopentag = function (node) {
			...
			let n = _pvnode.emptyINode(nodeType, nodeName, _pvnode.emptyAttrMap(attrs));
			if (last) {
				last = _pvnode.push(last, [nodeName, n]);
				parents.push(last);
			}
			last = n;
		}.bind(this);
		saxParser.onclosetag = function () {
			...
			last = _pvnode.finalize(last);
			last = parents.pop();
		};
		saxParser.onend = function () {
			saxParser.ended = true;
			return this.emit("end", _pvnode.finalize(last));
		}.bind(this);
		var ontext = function (val, type = 3) {
			if (/\S/.test(val)) {
				let name = _pvnode.count(last) + 1;
				let n = _pvnode.ivalue(type, name, val);
				last = _pvnode.push(last, [name, n]);
			}
		};
		...
		saxParser.ondoctype = function (value) {
			last = _pvnode.setAttribute(last, "DOCTYPE", value);
		};
		saxParser.onprocessinginstruction = function (pi) {
			last = _pvnode.setAttribute(last, pi.name, pi.body);
		};
