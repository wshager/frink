...
class Parser extends _events.EventEmitter {
	...
	reset() {
		var doc = (0, _vnode.emptyVNode)(9, "#document", -1, (0, _vnode.emptyAttrMap)()),
		    depth = 0;
		var last = doc,
		    parents = [];
		...
		saxParser.onopentag = function (node) {
			...
			let attrMap = (0, _vnode.emptyAttrMap)();
			for (key in ref) {
			...
			let n = (0, _vnode.emptyVNode)(nodeType, nodeName, depth, attrMap.endMutation(true));
			if (last) {
		...
		var ontext = function (value, type = 3) {
			if (/\S/.test(value)) {
				...
				let n = new _vnode.Value(type, name, value, depth + 1);
