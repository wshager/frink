...
class Parser extends _events.EventEmitter {
	...
	reset() {
		var last = _persist.emptyINode(9, "#document", _persist.emptyAttrMap());
		var parents = [];
		...
		saxParser.onopentag = function (node) {
			...
			let n = _persist.emptyINode(nodeType, nodeName, _persist.emptyAttrMap(attrs));
			if (last) {
				last = _persist.push(last, [nodeName, n]);
				parents.push(last);
			}
			last = n;
		}.bind(this);
		saxParser.onclosetag = function () {
			...
			last = _persist.finalize(last);
			last = parents.pop();
		};
		saxParser.onend = function () {
			saxParser.ended = true;
			return this.emit("end", _persist.finalize(last));
		}.bind(this);
		var ontext = function (val, type = 3) {
			if (/\S/.test(val)) {
				let name = _persist.count(last) + 1;
				let n = _persist.ivalue(type, name, val);
				last = _persist.push(last, [name, n]);
			}
		};
		...
		saxParser.ondoctype = function (value) {
			last = _persist.setAttribute(last, "DOCTYPE", value);
		};
		saxParser.onprocessinginstruction = function (pi) {
			last = _persist.setAttribute(last, pi.name, pi.body);
		};
