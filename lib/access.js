...
function attribute(qname, node) {
	if (arguments.length < 2) return _seq.seq(Axis(attribute.bind(null, qname), 2));
	var hasWildcard = /\*/.test(qname);
...
function _selectImpl(node, paths) {
	...
	var axis = self(),
	    directAccess;
	// process strings (can this be combined?)
	paths = _transducers.transform(paths,_transducers.compose(_transducers.forEach(function (path) {
		if (typeof path == "string") {
			var at = /^@/.test(path);
			if (at) path = path.substring(1);
			return at ? attribute(path) : _seq.seq(child(),element(path));
		}
		return _seq.seq(path);
	}),_transducers.cat));
	...
	const process = n => {
		...
		// handle directAccess
		if(directAccess && isVNode(list)) {
			list = list.get(directAccess);
		}
		return _transducers.into(list, composed,_seq.seq());
	};
	if (_seq.isSeq(node)) {
