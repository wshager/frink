...
function self(f) {
	if (f.name !== "transForEach") f = _transducers.forEach(f);
...
function _attrGet(key, node) {
	...
	return _transducers.forEach(entries, kv => node.vnode(node.ivalue(2, kv[0], kv[1]), node.parent, node.depth + 1, node.indexInParent));
}
