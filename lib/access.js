...
function nextNode(node) {
	...
	if (type != 17 && vnode.count() > 0) {
		...
		node = new _vnode.Node(vnode, vnode._type, vnode._name, vnode._value, path, index, parent, indexInParent);
		node.path.push(node);
		return node;
	} else {
		indexInParent++;
		...
		if (parent.count() == indexInParent) {
			...
			node = new _vnode.Step(vnode, path, index, node.parent, node.indexInParent);
			node.path.push(node);
			return node;
		} else {
...
function nextSibling(node) {
	...
	if (next) return new _vnode.Node(next, next.type, next.name, node.path, -1, pvnode, i);
}
...
function* children(node) {
	...
	while (!iter.done) {
		...
		yield new _vnode.Node(c, c.type, c.name, node.path, -1, vnode, i);
		i++;
	}
}
...
function childrenByName(node, name) {
	...
	if (hasWildcard) {
		...
		return new _seq.LazySeq((0, _transducers.filter)(node.vnode, c => regex.test(c.name))).map((c, i) => new _vnode.Node(c, c._type, c._name, c._value, node.path, -1, node.vnode, i));
	} else {
		let entry = node.vnode.get(name);
		if (entry.constructor == Array) {
			return new _seq.LazySeq(entry).map(c => new _vnode.Node(c, c._type, c._name, c._value, node.path, -1, node.vnode));
		} else {
			return new _seq.LazySeq([new _vnode.Node(entry, entry._type, entry._name, entry._value, node.path, -1, node.vnode)]);
		}
	}
}
...
function getDoc(node) {
	...
	return new _vnode.Node(vnode, vnode._type, vnode._name, null, node.path, -1);
}
