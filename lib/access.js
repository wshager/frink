...
function stringify(input) {
	...
	try {
		for (var _iterator = new VDoc(input)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			if (type == 1) {
				str += "<" + _node.name;
				str = (0, _seq.foldLeft)(_node.attrEntries(), str, attrFunc);
				if (!_node.count()) str += "/";
				str += ">";
			} else if (type == 3) {
				str += _node.toString();
			} else if (type == 9) {
				str += (0, _seq.foldLeft)(_node.attrEntries(), str, docAttrFunc);
			} else if (type == 17) {
				str += "</" + _node.name + ">";
			}
		}
...
function _attrGet(key, node) {
	...
	return into(entries, function (kv) {
...
function select($node) {
	...
	return (0, _seq.seq)(paths).concatMap(function (path) {
		...
		return _seq.seq($node).concatMap(function (node) {
			return f(bound(node));
		});
