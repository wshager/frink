...
function children($node) {
  const cx = this;
  return _l3n.ensureDoc.bind(cx)($node).concatMap(node => {
    const vnode = _vnodeFromCx(cx, node);

    const values = node.type == 2 ? [node.inode] : node.values();
    const depth = node.depth + 1;
    return (0, _seq.seq)(values).map((inode, idx) => vnode(inode, node, depth, idx + 1));
  });
}
...
function parent($node) {
  ...
  var cx = this;
  return _l3n.ensureDoc.bind(cx)($node).concatMap(node => (0, _seq.seq)(node.parent));
}

function self($f) {
  return (0, _card.zeroOrOne)($f).map(f => {
...
function _attrGet(key, $node) {
  return (0, _seq.seq)($node).concatMap(node => {
    var entries;
...
function attribute($qname) {
  ...
  return (0, _card.exactlyOne)($qname).map(qname => {
...
function child($f) {
  const cx = this;
  return (0, _seq.forEach)((0, _card.zeroOrOne)($f), f => {
...
function siblingsOrSelf($node) {
  var cx = this;
  return _l3n.ensureDoc.bind(cx)($node).concatMap(node => children.bind(cx)(node.parent));
}
...
function select($node, ...paths) {
  ...
  return (0, _seq.seq)((0, _seq.forEach)(paths, path => _axify(path))) // we're passing $node here, because we want to update it every iteration
  .map(path => $node => {
    ...
    return (0, _seq.seq)($node).concatMap(node => f(bound(node)));
  }).reduce(($node, changeFn) => changeFn($node), boundEnsureDoc($node)).concatAll();
}
...
function name($a) {
  ...
  return (0, _seq.seq)($a).concatMap(a => {
    if (!isVNode(a)) {
      return (0, _error.error)("XXX", "This is not a node");
    }

    if (a.type != 1) return (0, _seq.seq)();
    return (0, _seq.seq)(a.name);
  });
}
