...
function self(f) {
	return Axis(node => node ? _seq.seq([node]) : _seq.seq(), f);
}
...
function attribute(qname) {
	...
	// TODO iterator!
	var attrGet = node => _transducers.forEach(node.attrEntries(), v => node.vnode(node.ivalue(2, v[0], v[1]), node.parent, node.depth + 1, node.indexInParent));
	...
	return Axis(attrGet, _transducers.filter(f), 2);
}
...
function select(node, ...paths) {
	...
	while (paths.length > 0) {
		...
		cur = _seq.isSeq(cur) ? _transducers.forEach(cur, f) : f(cur);
	}
	return cur;
}
...
function _selectImpl(path) {
	...
	var f = path.f,
	    g = path.g;
	if (!attr) f = _transducers.compose(f, _transducers.filter(_comparer()));
	// TODO make lazy!
	// retrieve the iterator from the curent node and apply filter
	return n => _transducers.into(g(bed(n)), f, _seq.seq());
}
