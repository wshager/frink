...
function _attrGet(node, key = null) {
	if (key !== null) {
...
function attribute(qname, node) {
	...
	var hasWildcard = /\*/.test(qname);
	var f = _transducers.forEach(v => {
		return node.vnode(node.ivalue(2, v[0], v[1]), node.parent, node.depth + 1, node.indexInParent);
	});
	if (hasWildcard) {
		var regex = new RegExp(qname.replace(/\*/, "(\\w[\\w0-9-_]*)"));
//		console.log(node)
		f = _transducers.compose(_transducers.filter(n => {
			//console.log("attrNode",n)
			return regex.test(n[0]);
		}),f);
	}
...
function _selectImpl(node, path) {
	...
	if(_seq.isSeq(node)) {
		return _transducers.transform(node, _transducers.compose(_transducers.forEach(process)));
	} else {
		return process(node);
	}
}
