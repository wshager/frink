...
const _isList = n => isVNode(n) && n.type == 5; //const _isMap = n => isVNode(n) && n.type == 6;
...
function _attrGet(key, $node) {
  return (0, _seq.switchMap)($node, node => {
    ...
    return (0, _seq.forEach)((0, _seq.from)(entries), function ([k, v]) {
...
function select($node, ...paths) {
  ...
  return (0, _seq.foldLeft)((0, _seq.forEach)((0, _seq.forEach)((0, _seq.from)(paths), path => _axify(path)), // we're passing $node here, because we want to update it every iteration
  path => $node => {
