...
const last = exports.last = _transducers.filter(function(...a){
	console.log("M",a)
	return n.parent.size == n.indexInParent
});
...
function _selectImpl(node, path) {
	...
	//console.log(list)
	return _transducers.transform(list, _transducers.compose(_transducers.forEach(process), (n, k, i, z) => {
		return !isNode(n) || attr ? _transducers.cat(_seq.isSeq(n) ? n : [n], k, i, z) : _transducers.distinctCat(_comparer())(n, k, i, z)
	}));
}
