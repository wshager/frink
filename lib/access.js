...
Step.prototype.__is_VNode = true;
...
function _attrGet(key, $node) {
	return _seq.seq($node).concatMap(node => {
		var entries;
		if (key !== null) {
	...
	// we're passing $node here, because we want to update it every iteration
		.map(function (path) {
			...
			// TODO skip self
				...
				if (!skipCompare) f = (0, _pipe.pipe)(f, (0, _seq.filter)(_comparer()));
...
function name($a) {
	return _seq.seq($a).concatMap(a => {
		if (!isVNode(a)) {
			return _error.error("XXX","This is not a node");
		}
		if(a.type != 1) return _seq.seq();
		return _seq.seq(a.name);
	});
}
