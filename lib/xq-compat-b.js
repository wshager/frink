...
xqc.unwrap$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $cur = a[0];
	let $tmp = a[1];
	let $r = $tmp("r");
	let $d = $tmp("d");
	...
	let $ocur = n.if(n.gt($osize, 0), ($ocur, test) => {
		if (test) {
			return $o($osize);
		} else {
			return map.map();
	...
	let $t = $cur("t");
	let $v = $cur("v");
	let $type = n.if(n.eq($t, 1), ($type, test) => {
		if (test) {
	...
	let $has = n.if(n.eq($ot, 1), ($has, test) => {
		if (test) {
	...
	return n.if(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.eq($osize, 0), $pass), $hasAss), $isBody), $isLetInElse), $hasTypesig), $hasParam), $hasAf), $matching), $closeThen), $isXret), $hasXret), $isX), $hasX), $hasXfor), $hasConstr), $hasTuple), n.eq($has, 11)), (test) => {
		if (test) { /*            let $nu := console:log("stop")*/
			let $tpl = n.if(n.or($hasX, $hasXass), ($tpl, test) => {
				if (test) {
			...
			let $d = n.if(n.or($hasX, $hasXass), ($d, test) => {
				if (test) {
									...
									return n.if(n.eq($v, 222), (test) => {
										if (test) {
											...
											return a.foldLeftAt($p, $tpl, n.typed(n.function(n.seq(n.item(), n.item(), n.item()), n.item()), ((...a) => {
												...
												let $cur = a[1];
												let $i = a[2];
												return n.seq($pre, xqc.tpl(10, $d, "$"), xqc.tpl(1, $d, 1), xqc.tpl(4, n.add($d, 1), $cur), xqc.tpl(3, n.add($d, 1), ","), xqc.tpl(4, n.add($d, 1), "$"), xqc.tpl(1, n.add($d, 1), 1), xqc.tpl(8, n.add($d, 2), $i), xqc.tpl(2, n.add($d, 1), 2), xqc.tpl(2, $d, 2), n.if(n.or($isLet, n.eq($type, 225)), ($i, test) => {
													if (test) {
														return n.seq();
													} else {
														return xqc.tpl(3, $d, ",");
															...
															return n.if(n.or($isBody, $hasConstr), (test) => {
																if (test) {
																	...
																	return n.if(n.or($hasAf, $isClose), (test) => {
																		if (test) {
																			let $closeCurly = n.if(n.eq($has, 3), ($closeCurly, test) => {
																				if (test) {
																			...
																			return xqc.tpl($t, $d, n.if(n.boolean($closeCurly), ($closeCurly, test) => {
																				if (test) {
																					return $v;
																				} else {
																					return 2;
			...
			let $o = n.if(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or(n.or($hasParam, $hasConstr), n.seq(n.and($hasAss, n.ne(xqc.guardedGet($r, $size)("t"), 3)))), $isBody), $hasAf), $matching), $closeThen), $isXret), $isX), $hasTuple), ($o, test) => {
				if (test) {
					return a.pop($o);
				} else {
					return $o;
			...
			let $r = n.if(n.and($hasOp, n.seq(n.or(n.gt($ov, 3000), $hasConstrType))), ($r, test) => {
				if (test) {
					return $r;
				} else {
...
xqc.rtp$9 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item(), n.item(), n.item(), n.item(), n.item(), n.item()), n.item()), ((...a) => {
	let $tmp = a[0];
	let $r = a[1];
	let $d = a[2];
	let $o = a[3];
...
xqc.binOp$5 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item(), n.item()), n.item()), ((...a) => {
	let $t = a[0];
	let $v = a[1];
	let $tmp = a[2];
	let $ocur = a[3];
	...
	let $precedingOp = n.if(n.and($hasPreOp, $ocur("v")), ($precedingOp, test) => {
		if (test) {
	...
	let $split = n.if(map.contains($i, $d), ($split, test) => {
		if (test) {
			return $i($d);
		} else {
	...
	let $left = /*      if($v eq 1901 and $has-op and $ocur("v") eq 1901) then*/ /*          []*/ /*      else */ n.if(n.boolean($precedingOp), ($left, test) => {
		if (test) {
...
xqc.processOpen$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $v = a[0];
	let $tmp = a[1];
	let $t = 1;
	let $r = $tmp("r");
	...
	return n.if(n.eq($v, 2001), (test) => {
		if (test) {
			...
			let $it = n.if(n.or(n.eq($size, 0), n.seq(n.and(n.geq(xqc.guardedGet($r, $size)("t"), n.seq(1, 3, 6)), n.eq($hasSelect, n.false())))), ($it, test) => {
				if (test) {
					return 2004;
				} else {
					return 2001;
			...
			return n.if(n.eq($v, 3), (test) => {
				if (test) {
					...
					let $o = n.if(n.boolean($hasRettype), ($o, test) => {
						if (test) {
					...
					let $ocur = n.if(n.boolean($hasRettype), ($ocur, test) => {
						if (test) {
					...
					let $tpl = n.if(n.boolean($hasParams), ($tpl, test) => {
						if (test) {
							let $tpl = n.if(n.boolean($hasRettype), ($tpl, test) => {
								if (test) {
									return xqc.tpl(2, $d, 2);
								} else {
					...
					}, $tpl); /* remove constr type if not constr */
					return xqc.rtp($tmp, $r, n.if(n.boolean($hasParams), ($tpl, test) => {
						if (test) {
							return $d;
						} else {
							return n.add($d, 1);
						}
					}, $tpl), $o, $i, n.if(n.boolean($hasParams), ($tpl, test) => {
						if (test) {
							return array.array();
						} else {
							return $p;
						}
					}, $tpl), $tpl, $hasConstrType, n.if(n.boolean($hasParams), ($tpl, test) => {
						if (test) {
							return n.seq();
						} else {
							return n.if(n.boolean($hasConstrType), (test) => {
					...
					let $r = n.if(n.boolean($hasLambda), ($r, test) => {
						if (test) {
							return a.pop($r);
						} else {
							return $r;
					...
					let $tpl = n.if(n.boolean($hasFunc), ($tpl, test) => {
						if (test) {
					...
					return xqc.rtp($tmp, $r, n.if(n.boolean($hasFunc), ($tpl, test) => {
						if (test) {
							return n.add($d, 2);
						} else {
							return n.add($d, 1);
						}
					}, $tpl), $o, $i, $p, $tpl, n.or($hasFunc, $hasConstrType), n.if(n.boolean($hasFunc), ($tpl, test) => {
						if (test) {
...
xqc.processComma$1 = n.typed(n.function(n.seq(n.item()), n.item()), ((...a) => {
	let $tmp = a[0];
	...
	return n.if(n.or(n.seq(n.and($hasOp, n.eq($ocur("v"), 3006))), n.seq(n.and(n.eq($ocur("t"), 1), n.eq($ocur("v"), 2004)))), (test) => {
		if (test) {
			let $cur = xqc.tpl($t, $d, $v);
			let $tpl = n.if(n.eq($ocur("v"), 3006), ($tpl, test) => {
				if (test) {
					return n.seq(xqc.tpl(4, $d, "item"), xqc.tpl(1, $d, 1), xqc.tpl(2, $d, 2), $cur);
				} else {
					return $cur;
			...
			let $o = $tmp("o");
			let $osize = array.size($o);
			let $ocur = n.if(n.boolean($osize), ($ocur, test) => {
				if (test) {
					return $o($osize);
				} else {
					return map.map();
			...
			let $tpl = n.if(n.boolean($hasAss), ($tpl, test) => {
				if (test) {
			...
			return xqc.rtp($tmp, $tmp("r"), n.if(n.boolean($hasAss), ($tpl, test) => {
				if (test) {
					return n.add($d, 1);
				} else {
					return $d;
				}
			}, $tpl), $tmp("o"), $tmp("i"), $tmp("p"), $tpl, n.seq(), n.if(n.boolean($hasAss), ($tpl, test) => {
				if (test) {
					return xqc.tpl(4, $d, 209);
				} else {
					return n.seq();
...
xqc.processOp$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $v = a[0];
	let $tmp = a[1];
	let $t = 4;
	let $r = $tmp("r");
												...
												if (test) {
													...
													return n.if(n.eq($v, 2106), (test) => {
														if (test) { /* check if o contains declare, otherwise its anonymous */
															...
															let $tpl = n.if(n.boolean($hasDecl), ($tpl, test) => {
																if (test) {
																	return n.seq(xqc.tpl(10, $d, "$>"), xqc.tpl(1, $d, 1));
																} else {
																	return xqc.tpl($t, $d, $v);
															...
															return n.if(n.eq($v, 2400), (test) => {
																if (test) {
																	...
																	return xqc.rtp($tmp, $r, $d, $o, $i, $p, n.if(n.boolean($hasParams), ($hasParams, test) => {
																		if (test) {
																			return n.seq();
																		} else {
																			return xqc.tpl(3, $d, ",");
																		}
																	}, $hasParams), n.seq(), xqc.tpl($t, $d, $v));
																} else {
																				...
																				if (test) {
																					...
																					return n.if(n.eq($v, 209), (test) => {
																						if (test) { /* TODO check if o contains something that prevents creating a new let-ret-seq */ /* remove entry */
																							...
																							let $open = n.if(n.empty($ocur("t")), ($open, test) => {
																								if (test) {
																																				...
																																				if (test) {
																																					return n.if(n.eq($size, 0), (test) => {
																																						if (test) { /* unary-op: insert op + parens */
																																							...
																																							let $isOcc = n.if(n.geq($v, n.seq(802, 904, 2003)), ($isOcc, test) => {
																																								if (test) {
...
xqc.processVar$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $v = a[0];
	let $tmp = a[1];
	let $t = 5;
	let $r = $tmp("r");
	...
	$v = n.replace($v, "^\\$", "");
	let $tpl = n.if(n.or(n.or($isParam, $hasXass), $hasX), ($tpl, test) => {
		if (test) {
			return n.seq();
		} else {
			return n.if(n.boolean($hasAss), (test) => {
	...
	return xqc.rtp($tmp, $r, $d, $o, $i, $p, $tpl, n.seq(), n.seq(), n.if(n.or(n.or($isParam, $hasXass), $hasX), ($tpl, test) => {
		if (test) {
			return $v;
		} else {
			return n.seq();
...
xqc.processQname$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $v = a[0];
	let $tmp = a[1];
	let $t = 6;
	let $r = $tmp("r");
	...
	return n.if(n.and($hasOp, n.geq($ocur("v"), n.seq(2102, 2105))), (test) => {
		if (test) { /*                let $nu := console:log(map {"auto-constr":$ocur,"i":$i}) return*/
			...
			let $tpl = n.if(n.and($hasOp, n.eq($ocur("v"), 2108)), ($tpl, test) => {
				if (test) {
 ...
 */
xqc.process$2 = n.typed(n.function(n.seq(map.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore())), map.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore()))), n.item()), ((...a) => {
	let $cur = a[0];
	let $tmp = a[1];
	let $t = $cur("t");
	let $v = $cur("v");
														...
														if (test) {
															...
															return n.if(n.eq($t, 10), (test) => {
																if (test) {
																	...
																	let $tpl = n.if(n.boolean($isFor), ($tpl, test) => {
																		if (test) {
																			return n.seq();
																		} else {
																			return xqc.tpl($t, $d, $v);
...
xqc.toL3$5 = n.typed(n.function(n.seq(n.item(), n.item(), n.item(), n.item(), n.item()), n.item()), ((...a) => {
	...
	let $s = n.if(n.geq($t, 1), ($s, test) => {
		if (test) {
				...
				if (test) {
					...
					return n.if(n.eq($v, 1), (test) => {
						if (test) { /* TODO check for last operator */
							let $last = n.if(n.gt($at, 1), ($last, test) => {
								if (test) {
			...
			return n.if(n.eq($t, 2), (test) => {
				if (test) {
					let $next = n.if(n.lt($at, $size), ($next, test) => {
						if (test) {
...
xqc.normalizeQuery$2 = n.typed(n.function(n.seq(n.string(), map.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore()))), n.item()), ((...a) => {
	let $query = a[0];
	...
	return xqc.normalizeQueryB(xqc.toBuffer(n.concat($query, " ")), $params);
}));
xqc.normalizeQueryB$2 = n.typed(n.function(n.seq(n.occurs(n.string(), n.zeroOrMore()), map.map(n.anyAtomicType(), n.occurs(n.item(), n.zeroOrMore()))), n.item()), ((...a) => {
	let $buffer = a[0];
	let $params = a[1]; /* TODO strip comments for now */
	let $tmp = xqc.analyzeChars($buffer, n.eq($params("$compat"), "xquery")) /*    let $nu := console:log($tmp)*/ ;
	let $normalform = xqc.unwrap(map.map(n.pair("t", 0)), $tmp)("r");
	...
	return n.if(n.eq($output, "rdl"), (test) => {
		if (test) {
			return a.foldLeft($normalform, "", n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
				...
				let $t = $entry("t");
				let $v = $entry("v");
				return n.concat($pre, n.if(n.geq($t, n.seq(1, 2)), ($v, test) => {
					if (test) {
						return xqc.operators($v);
					} else {
						return n.if(n.eq($t, 7), (test) => {
...
xqc.analyzeChars$2 = n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
	let $chars = a[0];
	let $xqCompat = a[1];
	return xqc.analyzeChars(n.tail($chars), $xqCompat, map.map(n.pair("char", n.head($chars)), n.pair("type", 0), n.pair("buffer", n.seq()), n.pair("string", 0), n.pair("var", n.false()), n.pair("ws", n.false()), n.pair("number", n.false()), n.pair("comment", n.false()), n.pair("opentag", n.false()), n.pair("closetag", n.false()), n.pair("attrkey", n.false()), n.pair("attrval", n.false()), n.pair("enc-expr", n.false()), n.pair("has-quot", 0), n.pair("opencount", 0), n.pair("r", array.array()), n.pair("d", 1), n.pair("o", array.array()), n.pair("i", map.map()), n.pair("p", array.array())));
}));
xqc.analyzeChars$3 = n.typed(n.function(n.seq(n.item(), n.item(), n.item()), n.item()), ((...a) => {
	let $chars = a[0];
	let $xqCompat = a[1];
	let $flags = a[2]; /* if the type changes, flush the buffer */
	/* TODO:
	 * WS for XML
	 * type 10 instead of 0 for enclosed expression
	 * revert to pair checking, tokenize all chars here
	 */
	return n.foldLeft($chars, $flags, n.typed(n.function(n.seq(n.item(), n.item()), n.item()), ((...a) => {
		let $flags = a[0];
		let $next = a[1];
		let $char = $flags("char");
		...
		let $type = n.if(n.ne($string, 0), ($type, test) => {
		...
		let $zero = n.if(n.geq(n.seq($comment, $opentag, $closetag, $attrkey), n.true()), ($zero, test) => {
			if (test) {
				return n.false();
			} else {
		...
		let $flag = n.if(n.boolean($number), ($flag, test) => {
			if (test) {
				return n.seq();
			} else {
		...
		let $emitBuffer = n.if(n.boolean($flag), ($emitBuffer, test) => {
			if (test) {
		...
		let $tpl = n.if(n.or(n.or(n.geq($flag, n.seq(2, 4, 6, 7, 8, 9, 10)), $wasNumber), $wasVar), ($tpl, test) => {
			if (test) {
				return n.seq();
			} else {
		...
		$flags = n.if(n.exists($tpl), ($flags, test) => {
			if (test) {
		...
		let $fromBuf = n.if(n.and(n.and(n.empty($flag), $emitBuffer), $zero), ($fromBuf, test) => {
			if (test) {
		...
		$flags = n.if(n.eq($type, 5), ($flags, test) => {
			if (test) {
				return map.put($flags, "i", map.map());
			} else {
				return $flags;
			}
		}, $flags);
...
xqc.unwrap = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.unwrap$2.apply(null, a);
};
xqc.rtp = (...a) => {
	const len = a.length;
	...
	if (len == 10) return xqc.rtp$10.apply(null, a);
};
xqc.binOp = (...a) => {
	const len = a.length;
	if (len == 5) return xqc.binOp$5.apply(null, a);
};
xqc.processOpen = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.processOpen$2.apply(null, a);
};
xqc.processComma = (...a) => {
	const len = a.length;
	if (len == 1) return xqc.processComma$1.apply(null, a);
};
xqc.processOp = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.processOp$2.apply(null, a);
};
xqc.processVar = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.processVar$2.apply(null, a);
};
xqc.processQname = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.processQname$2.apply(null, a);
};
xqc.process = (...a) => {
	const len = a.length;
	if (len == 2) return xqc.process$2.apply(null, a);
};
xqc.toL3 = (...a) => {
	const len = a.length;
...
xqc.analyzeChars = (...a) => {
	...
	if (len == 3) return xqc.analyzeChars$3.apply(null, a);
};
module.exports = xqc;
