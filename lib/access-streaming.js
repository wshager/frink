...
function children($node, axis) {
	...
	var _hasAncestor = function _hasAncestor(node, maybeAncestor) {
		while (node.parent) {
...
function child($f) {
	...
	return (0, _seq.seq)($f).map(function (f) {
		...
		var stepper = function stepper($node) {
			return children.bind(cx || _this)($node, axis);
		};
		axis.g = stepper;
		return axis;
	});
