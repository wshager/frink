...
function construct() {
	...
	if (l == 1) {
		...
		if (isMap(s)) return mapSeq((0, _seq.of)(s));
		if(Array.isArray(s)) return mapSeq(fromEntries(s));
		if ((0, _util.isObject)(s)) return mapSeq((0, _seq.of)(fromEntries(Object.entries(s))));
		// TODO VNode conversion + detect tuple
		//if ((0, _access.map)()(s)) return mapSeq((0, _seq.of)(s.toMap()));
...
var merge = exports.merge = function merge($m) {
	...
	return mapSeq($m.reduce(function (pre, cur) {
		// TODO force persistent cx
		//if ((0, _access.map)()(cur)) cur = cur.toMap();
		if (!isMap(cur)) return (0, _error.error)("XPTY0004", "One of the items for map:merge is not a map.");
		return fromEntries(cur, pre);
	}, _create()));
};

function put($m, $k, $v) {
	return mapSeq(_seq.forEach((0, _seq.exactlyOne)($m),function (m) {
		$v = (0, _seq.seq)($v);
		return _seq.forEach((0, _seq.exactlyOne)($k),function (k) {
			return _seq.forEach((0, _seq.isExactlyOne)($v),function (test) {
				return test ? $v.map(function (v) {
					return m.set(k, v);
				}) : (0, _seq.seq)(m.set(k, $v));
			});
		});
	}));
}
...
function entry($k, $v) {
	...
	$v = (0, _seq.seq)($v);
	return (0, _seq.seq)($k).concatMap(function (k) {
		var m = _create();
		return (0, _seq.isExactlyOne)($v).concatMap(function (test) {
			return test ? $v.map(function (v) {
				return m.set(k, v);
			}) : (0, _seq.seq)(m.set(k, $v));
		});
...
function remove($m, $k) {
	return (0, _seq.exactlyOne)($m).concatMap(function (m) {
		return (0, _seq.exactlyOne)($k).map(function (k) {
			return m.delete(k);
		});
	});
}
