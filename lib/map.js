...
function map(...a) {
	...
	if (l == 1 && (0, _seq.isSeq)(a[0])) {
		a = a[0];
		if ((0, _seq.isEmpty)(a)) return m.endMutation();
	}
...
function put($map, $k, $v) {
	...
	return map.set(k, (0, _seq.isSeq)($v) && $v.size > 1 ? $v : (0, _seq.first)($v));
}

function keys($map) {
	return (0, _seq.seq)(Array.from((0, _seq.first)($map).keys()));
}
...
function forEachEntry($map, $fn) {
	let map = (0, _seq.first)($map);
	let fn = (0, _seq.first)($fn);
	var ret = (0, _seq.seq)();
...
function entry(...a) {
	...
	if (a.length != 2) return (0, _error.error)("err:XPST0017", "Number of arguments of function map.entry doesn't match function signature (expected 2, got " + a.length + ")");
	var m = ohamt.empty,
	    k = (0, _seq.first)(a[0]),
	    v = a[1];
	return m.set(k.valueOf(), (0, _seq.isSeq)(v) && v.size > 1 ? v : (0, _seq.first)(v));
}

function get($map, $key) {
	var map = (0, _seq.first)($map);
	var k = (0, _seq.first)($key).valueOf();
	var v = map.get(k);
