...
function map(...a) {
	...
	return _transducers.into(a, _transducers.compose(_transducers.filter(x => {
		//console.log("x",x.size,x.toJS());
		return x;
	}),_transducers.cat), m).endMutation();
}
...
function put($map, $k, $v) {
	...
	return map.set(k, _seq.isSeq($v) && $v.size != 1 ? $v : _seq.first($v).valueOf());
}

function keys($map) {
	var map = _seq.first($map);
	return _seq.seq(ohamt.keys(map));
}
...
function entry(...a) {
	...
	var m = ohamt.empty,
	    k = _seq.first(a[0]),
	    v = a[1];
	return m.set(k, _seq.isSeq(v) && v.size != 1 ? v : _seq.first(v).valueOf());
}

function get($map, $key) {
	var map = _seq.first($map);
	var k = _seq.first($key);
	var v = map.get(k);
