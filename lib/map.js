...
OrderedMap.prototype.call = function($,$k,$v) {
	...
	if(len == 1) return get(this,$k);
	if(len == 2) return set(this,$k,$v);
};
...
function keys($m) {
	return _seq.forEach(_seq.exactlyOne($m),m => {
		return m.keys();
	});
}
...
function forEachEntry($m, $fn) {
	return _seq.forEach((0, _seq.exactlyOne)($fn),function (fn) {
		return _seq.forEach((0, _seq.exactlyOne)($m),function (m) {
			return _seq.forEach(_seq.from(m.entries()),function([k,v]) {
				return fn(k, v);
			});
		});
	});
